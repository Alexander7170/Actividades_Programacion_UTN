...
def analizar_collidepoint_jugar(rect_opcion:dict, estado_pantalla:dict, evento:pygame.event.Event, respuesta:str)->bool:
    """
    ...
    coliciono_con_opcion = False
    if rect_opcion["a"].collidepoint(posicion_click):
        respuesta = "a"
    elif rect_opcion["b"].collidepoint(posicion_click):
        respuesta = "b"
    elif rect_opcion["c"].collidepoint(posicion_click):
        respuesta = "c"
    elif rect_opcion["volver_menu"].collidepoint(posicion_click):
        estado_pantalla[JUGANDO] = False
        estado_pantalla[VIENDO_MENU] = True
    if respuesta != "":
        coliciono_con_opcion = True
    return respuesta, coliciono_con_opcion

def analizar_tecla(evento:pygame.event.Event, texto:str, maximo_caracteres:int, estado_usuario:dict)->str:
    """
...
def cambiar_estado_pantalla_click(rect_opcion: dict, evento:pygame.event.Event, estado_usuario:dict, inicializacion:dict):
    """
...
def imprimir_score(pantalla:pygame.Surface,matriz_imagenes:list, color:tuple):
    """
...
def analizar_collidepoint_score(rect_opcion:dict, eventos_pantalla:dict, evento):
    posicion_click = evento.pos
    if rect_opcion["volver_menu"].collidepoint(posicion_click):
        eventos_pantalla[VIENDO_SCORE] = False
        eventos_pantalla[VIENDO_MENU] = True

def manejar_score(ruta:str)->list:
    texto_score = leer_score(ruta)
    ordenar_score_ascendentemente(texto_score)
    imagenes_texto = renderizar_score(texto_score,fuente_grande,COLOR_NEGRO)
