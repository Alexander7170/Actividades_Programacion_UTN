...
def obtener_posiciones_matriz_score(matriz_imagenes:list)->list:
    """
    Funcion que obtiene una ubicacion distinta para cada imagen en la matriz de imagenes
    Devuelve una tupla de tuplas en donde cada subtupla es la coordenada de una imagen
    """
    posiciones_imagenes = []
    eje_x = 30
    eje_y = 100
...
def obtener_imagenes_fondo_tablero(imagenes:dict,tablero:list,indice_usuario:int)->list:
    ...
    Devuelve la lista de imagenes ordenada en el orden del lista tablero
    """
    imagenes_fondo = []
    for i in range(len(tablero)):
        if i == indice_usuario:
